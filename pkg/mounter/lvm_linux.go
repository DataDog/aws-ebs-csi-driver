...
const (
	// lvmLockDir is a writable directory for LVM file locks.
	// /var/lib/kubelet is a hostPath mount with Bidirectional propagation,
	// so it's writable even with readOnlyRootFilesystem.
	lvmLockDir = "/var/lib/kubelet/lvm-lock"

	// lvmConfig overrides LVM defaults for containerized operation:
	// - locking_dir: writable path (default /run/lock/lvm is on read-only rootfs)
	// - archive/backup: disabled (/etc/lvm/archive is on read-only rootfs)
	lvmConfig = `global{locking_dir="` + lvmLockDir + `"} backup{archive=0 backup=0}`
)

// lvmCmd runs an LVM command with config overrides for containerized operation.
//
// The CSI node plugin container has readOnlyRootFilesystem and no hostPID, so:
//   - Default lock dir /run/lock is read-only → redirect to /var/lib/kubelet/lvm-lock
//   - /etc/lvm/archive is read-only → disable archive/backup
//   - /dev/<vg>/<lv> symlinks aren't created (no udev) → use /dev/mapper/ paths
//   - lvcreate uses --zero n since the dm device isn't visible for wiping
...
// LVPath returns the device-mapper path for a logical volume.
// We use /dev/mapper/ instead of /dev/<vg>/<lv> because the latter is a
// udev-created symlink that doesn't exist in containers without udev.
// LVM escapes hyphens in VG/LV names by doubling them in the mapper path.
