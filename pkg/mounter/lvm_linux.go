...
func (m *NodeMounter) CreateStripedLV(vgName string, lvName string, stripeCount int, stripeSize string, devices []string) error {
	for _, device := range devices {
		...
		output, err := m.Exec.Command("pvcreate", device).CombinedOutput()
		if err != nil {
	...
	output, err := m.Exec.Command("vgcreate", vgArgs...).CombinedOutput()
	if err != nil {
	...
	output, err = m.Exec.Command("lvcreate",
...
func (m *NodeMounter) ActivateVG(vgName string) error {
	...
	output, err := m.Exec.Command("vgchange", "-ay", vgName).CombinedOutput()
	if err != nil {
...
func (m *NodeMounter) DeactivateVG(vgName string) error {
	...
	output, err := m.Exec.Command("vgchange", "-an", vgName).CombinedOutput()
	if err != nil {
...
func (m *NodeMounter) RemoveVG(vgName string) error {
	...
	output, err := m.Exec.Command("lvchange", "-an", vgName).CombinedOutput()
	if err != nil {
	...
	output, err = m.Exec.Command("vgremove", "-f", vgName).CombinedOutput()
	if err != nil {
...
func (m *NodeMounter) RemovePVs(devices []string) error {
	...
	for _, device := range devices {
		...
		output, err := m.Exec.Command("pvremove", device).CombinedOutput()
		if err != nil {
...
func (m *NodeMounter) IsVGActive(vgName string) (bool, error) {
	output, err := m.Exec.Command("vgs", "--noheadings", "-o", "vg_name", vgName).CombinedOutput()
	if err != nil {
