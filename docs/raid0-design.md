...
### 11. LVM in a Container — Constraints and Workarounds

The CSI node plugin runs in a constrained container environment:
- `readOnlyRootFilesystem: true` — `/run/lock` and `/etc/lvm/archive` are read-only
- No `hostPID` — blocked by Gatekeeper (`nsenter -t 1` into host namespaces is not possible)
- `/dev` is a hostPath bind mount — kernel device-mapper nodes (`/dev/dm-X`) appear, but udev-managed symlinks (`/dev/<vg>/<lv>`) do not
- `/var/lib/kubelet` is a writable hostPath mount with Bidirectional propagation

**LVM config overrides** — all commands use `--config` to work around read-only paths:

```go
const lvmConfig = `global{locking_dir="/var/lib/kubelet/lvm-lock"} backup{archive=0 backup=0}`

func (m *NodeMounter) lvmCmd(cmd string, args ...string) ([]byte, error) {
    fullArgs := append([]string{"--config", lvmConfig}, args...)
...
| Problem | Workaround |
|---------|-----------|
| `/run/lock` read-only → LVM file locking fails | Redirect `locking_dir` to `/var/lib/kubelet/lvm-lock` (writable hostPath) |
| `/etc/lvm/archive` read-only → metadata archive fails | Disable archive/backup: `backup{archive=0 backup=0}` |
| `/dev/<vg>/<lv>` symlinks missing (no udev) | Use `/dev/mapper/<vg>-<lv>` paths (device-mapper creates these directly) |
| `lvcreate` fails wiping new LV (dm device not yet visible) | Use `--zero n` to skip wiping |

**Device path format**: LVM escapes hyphens in VG/LV names by doubling them. For VG `ebs-raid-abc` and LV `data`, the mapper path is `/dev/mapper/ebs--raid--abc-data`.

**Alternative approaches considered**:
- **nsenter into host namespaces** (TopoLVM/OpenEBS pattern): Requires `hostPID: true`, blocked by Gatekeeper
- **Create /run/lock at runtime**: `/run` is read-only overlay, `mkdir` fails
- **Mount emptyDir at /run/lock**: Requires DaemonSet spec change (platform-level), not driver-controlled
- **--nolocking**: Some LVM commands (e.g., `lvcreate`) fail with exit code 5 even with `--nolocking` because they still try to create archive dirs
